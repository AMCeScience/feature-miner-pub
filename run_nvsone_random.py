...
def run_models(data, fold):
  ...
  for i in range(0, len(X_train_sets)):
    single_obj = file_handle.load_classifier_part(fold, 'tfidf_random', i)
    ...
    file_handle.store_classifier_part(fold, 'tfidf_random', i, single_obj)
...
def fit_random_forest(X, y):
  ...
  grid_search = GridSearchCV(estimator = clf, param_grid = param_grid, cv = num_cv, n_jobs = 1, scoring = score)
...
def classify():
  model_runner = run_model.Run_model(run_models, 'n_vs_one_random', run_parallel = True)
